...
from models import Event, AttendanceRecord, Attendance, Student, db
...
def add_record(event_id):
    ...
    try:
        ...
        if not students:
            flash('No students found in database. Please add students first.', 'error')
            return redirect(url_for('events.records', event_id=event_id))
        
        # Use SQLAlchemy with explicit transaction management
        ...
        try:
            ...
            print(f"DEBUG: Created record with ID: {new_record.record_id}")
            
            # Verify the record exists in the database
            from sqlalchemy import text
            result = session.execute(text("SELECT record_id FROM attendancerecords WHERE record_id = :record_id"), 
                                   {"record_id": new_record.record_id})
            if not result.fetchone():
                raise Exception(f"Record with ID {new_record.record_id} was not found in database after commit")
            
            # Now create attendance entries for all students
            ...
            session.close()
        flash(f'Attendance record "{record_name}" created successfully with {len(students)} students initialized as Absent', 'success')
    except Exception as e:
        db.session.rollback()
...
@events_bp.route('/records/<int:record_id>/delete', methods=['POST'])
